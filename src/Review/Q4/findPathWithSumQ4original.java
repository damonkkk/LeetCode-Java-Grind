...
public class findPathWithSumQ4original {
    ...
    public List<TreeNode> findPathWithSum(TreeNode root, int targetSum){
        ...
        if(backtrack(root,targetSum,res)){
            return res;
        } else {
        return null;}
    }
    ...
    public boolean backtrack(TreeNode node, int targetSum, ArrayList<TreeNode> res){
        if(node == null){
            return false;
        }
        ...
        if(node.right == null && node.left == null && targetSum==0){
            ...
            if(backtrack(node.right,targetSum,res)){
                return true;
            };
            if(backtrack(node.left,targetSum,res)){
                return true;
            };
